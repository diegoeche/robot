...
module ultrasonic_sensor() {
    translate([0.6, 0, 0]) {
      import("/Users/wooga/Downloads/Ultrasonic_sensor_HC-SR04/sensor.stl");
    translate([0.6, 0, 0]) import("/Users/wooga/Downloads/Ultrasonic_sensor_HC-SR04/sensor.stl");
    }
}

module sensors_holder() {
    border = 5;
    sensor_length = 46;
    x_offset = 42;
    difference() {
       cube([width,50,15]);
       translate([border, -0.1, -0.1])
        cube([width - (border * 2), 50.1 - border, 15.1 - border]);
       translate([54, 43, -10]) rotate([-90, 180, 0]) scale(1.01) ultrasonic_sensor();
       translate([54 - sensor_length, x_offset, -10]) rotate([-90, 180, 90]) scale(1.01)ultrasonic_sensor();
       translate([54, x_offset - sensor_length - 0.6, -10]) rotate([-90, 180, -90]) scale(1.01) ultrasonic_sensor();
       //translate([6.5, 35, 19]) sbc_holes();
       translate ([56, 35 , 10]) cylinder(20, 2.5, 2.5, true); 
       translate ([7, 35 , 10]) cylinder(20, 2.5, 2.5, true);
       translate([9, 46, 15]) rotate([90,0,0]) cylinder(14, 5, 5, true);
       translate([3, -8, -5]) cube([15, 20, 30]);
       translate([51, 40, -0.1]) {
           difference() {
             cube([20,20,20]);
             cylinder(41, 10, 10, true);    
           }
       }
       translate([10, 40, -0.1]) {
